...
async function updateChain(chainConfig, newSignerAddress, ownerWallet) {
  ...
  try {
    ...
    // 读取当前签名地址
    const currentSigner = await contract.backendSigner();
    console.log('\n🔐 当前后端签名地址 (backendSigner):');
    ...
    const tx = await contract.setBackendSigner(newSignerAddress);
    console.log('\n📤 交易已发送:', tx.hash);
    console.log('   等待确认...');
    ...
    // 验证更新
    const updatedSigner = await contract.backendSigner();
    console.log('\n🔍 验证更新后的签名地址:');
