...
# 一键修复 Swap Mining 数据库表
# 使用方法: 复制整个命令到服务器终端执行

docker exec -it hocg04o8swccwggwc8kosc8g-133057446453 /bin/sh -c "cd /app && \
cat > fix-swap-mining-tables.cjs << 'EOF'
const Database = require('better-sqlite3');
const path = require('path');
const fs = require('fs');

console.log('🔧 [Swap Mining] 修复数据库表\\n');
...
try {
  ...
  \`);
  console.log('✅ user_claim_nonce 表已创建');
  
  // 3. 创建 user_swap_stats 表
  console.log('\\n🔨 创建 user_swap_stats 表...');
  db.exec(\`
  ...
  db.exec(\`
    ...
    CREATE INDEX IF NOT EXISTS idx_user_swap_stats_volume 
    ON user_swap_stats(total_volume_usdt);
  \`);
  console.log('✅ user_swap_stats 表已创建');
  
  // 4. 创建 swap_transactions 表
  console.log('\\n🔨 创建 swap_transactions 表...');
  db.exec(\`
  ...
  \`);
  console.log('✅ swap_transactions 表已创建');
...
echo '执行修复脚本...'
node fix-swap-mining-tables.cjs
"
