...
/**
 * @title EagleTokenRemote
 * @dev EAGLE Token for REMOTE CHAINS (BSC, ETH, Base, etc.)
 * 
 ...
 * 
 * Features:
 * - NO Initial Supply (tokens must bridge from Home Chain)
 * - Supports LayerZero cross-chain
 * - Router whitelist
 */
contract EagleTokenRemote is OFT, ERC20Burnable, ReentrancyGuard, AccessControl {
    ...
    /**
     * @dev Constructor for BSC/ETH/Base
     * @param _lzEndpoint LayerZero Endpoint for the specific chain
    ...
    constructor(
        ...
        // NO MINTING HERE - Tokens come from bridge
    }
    ...
    function addRouter(address router) external onlyRole(ROUTER_MANAGER_ROLE) {
        require(router != address(0), "EagleTokenRemote: zero address");
        require(!whitelistedRouters[router], "EagleTokenRemote: already whitelisted");
    ...
    function removeRouter(address router) external onlyRole(ROUTER_MANAGER_ROLE) {
        require(whitelistedRouters[router], "EagleTokenRemote: not whitelisted");
    ...
    function setBuyTaxes(
        ...
        require(_marketingTax <= MAX_TAX_RATE, "EagleTokenRemote: marketing tax too high");
        require(_burnTax <= MAX_TAX_RATE, "EagleTokenRemote: burn tax too high");
        require(_liquidityTax <= MAX_TAX_RATE, "EagleTokenRemote: liquidity tax too high");
    ...
    function setMarketingWallet(address _marketingWallet) external onlyRole(TAX_MANAGER_ROLE) {
        require(_marketingWallet != address(0), "EagleTokenRemote: zero address");
    ...
    function setLiquidityWallet(address _liquidityWallet) external onlyRole(TAX_MANAGER_ROLE) {
        require(_liquidityWallet != address(0), "EagleTokenRemote: zero address");
    ...
    function withdrawMarketingTax(uint256 amount) external onlyRole(TAX_MANAGER_ROLE) {
        require(amount > 0, "EagleTokenRemote: amount must be > 0");
        require(balanceOf(address(this)) >= amount, "EagleTokenRemote: insufficient balance");
    ...
    function withdrawAllMarketingTax() external onlyRole(TAX_MANAGER_ROLE) {
        ...
        require(balance > 0, "EagleTokenRemote: no marketing tax to withdraw");
